...
document.addEventListener('DOMContentLoaded', function (e) {
  ...
  if (dt_project_table) {
    ...
    var dt_project = new DataTable(dt_project_table, {
      ...
      layout: {
        topStart: {
          rowClass: 'row mx-md-2 my-0 justify-content-between',
          features: [tableTitle]
        },
        topEnd: {
          search: {
  ...
  if (dt_invoice_table) {
    let tableTitle = document.createElement('h5');
    tableTitle.classList.add('card-header', 'ps-0', 'mb-0', 'text-md-start', 'text-center');
      ...
      columnDefs: [
        ...
        },
        {
          ...
          render: (data, type, full, meta) => {
            ...
            // Creates full output for row
            const rowOutput = `<a href="/Invoice/Preview"><span>#${invoiceId}</span></a>`;
            return rowOutput;
          }
        },
        {
          ...
          render: (data, type, full, meta) => {
            return `
              <div class="d-flex align-items-center justify-content-center">
                <a href="javascript:;" class="btn btn-icon btn-text-secondary rounded-pill delete-record"><i class="icon-base ri ri-delete-bin-7-line icon-22px"></i></a>
                <a href="app-invoice-preview.html" class="btn btn-icon btn-text-secondary rounded-pill" data-bs-toggle="tooltip" title="Preview">
          ...
          features: [
            {
              buttons: [
                {
                  extend: 'collection',
                  className: 'btn btn-primary dropdown-toggle float-sm-end',
                  text: '<i class="icon-base ri ri-external-link-line icon-sm me-2"></i>Export',
                  buttons: [
                    {
                      extend: 'print',
